...
class MobileQQTest(object):
    ...
    def __init__(self, num):
        self.data = []
        self.num = num
        self.app = 'com.senseauto.healthdetect'
        self.activity = 'com.senseauto.healthdetect.ui.FirstActivity'
    ...
    def start_adb(self):
        '''运行adb命令，并记录启动耗时'''
        start = 'adb shell am start -S -W %s/%s' % (self.app, self.activity)
        data = re.findall(r'TotalTime:(.*?)WaitTime: (.*?)Complete',os.popen(start).read().replace('\n', ''))
        if len(data) == 0:
    ...
    def run_test_hot(self):
        ...
        self.data.clear()
        if self.check_devices() == True:
            os.popen('adb shell am start -S -W %s/%s' % (self.app, self.activity))
            time.sleep(3)
            for i in range(self.num):
